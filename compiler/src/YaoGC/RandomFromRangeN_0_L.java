 ...
 * 
 * inputs:
 *  - first 2^l bits: r_a randomness from party A
 *  - next 2^l bits: r_b randomness form party B
 *  - last l bits: n
 * outputs:
 *  - 2^l bits: r uniformly at random out of {0, 1, ..., (2^n)-1}
 *
 */
public class RandomFromRangeN_0_L extends CompositeCircuit {

	private int l, lpow;
	
	public RandomFromRangeN_0_L(int l) {
	...
	protected void connectWires() throws Exception {
		// n
		for(int i=0; i<l; i++){
			inputWires[2*lpow+i].connectTo(subCircuits[1].inputWires, i);
		}
		// r_a and r_b
		for(int i=0; i<lpow; i++){
			inputWires[i].connectTo(subCircuits[0].inputWires, i);
			inputWires[lpow+i].connectTo(subCircuits[0].inputWires, lpow+i);
		}
		for(int i=0; i<lpow; i++){
