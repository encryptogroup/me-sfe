...
public class OTExtSenderLowerMemAES {
	...
	public OTExtSenderLowerMemAES(int msgBitLength,
	...
	public void execProtocol(byte[][] zeroMsgs) throws Exception {
		...
		for (int i = 0; i < iterations; i++) {
			...
			for (int j = 0; j < SecurityParameter.k1; j++) {
				...
				if (s.testBit(j)) {
					Q.setRowData(j, CipherAES.decrypt(i, j,
							oneCph, elementsPerIteration, MAX_NUM_OF_PAIRS_PER_ITERATION));
				} else {
					Q.setRowData(j, CipherAES.decrypt(i, j,
							zeroCph, elementsPerIteration, MAX_NUM_OF_PAIRS_PER_ITERATION));
				}			
			}
			Qt = Q.transpose();
			for (int j = 0; j < elementsPerIteration; j++) {
				columnData = Qt.getRowData(j);
	...
	private void initialize() throws Exception {
		...
		rcver = new NPOTReceiverMultiThreading(SecurityParameter.k1, dis, dos);
		s = new BigInteger(SecurityParameter.k1, rnd);
		rcver.execProtocol(s);
		keys = rcver.getData();

		CipherAES.init(SecurityParameter.k1, SecurityParameter.k2, keys);
	}
}
