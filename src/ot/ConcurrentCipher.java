...
public final class ConcurrentCipher {
	...
	public byte[] encrypt(int j, byte[] key, byte[] msg, int msgLength){
		return LabelMath.xor(msg, getPaddingBytesOfLength(j, key, msgLength));
	}
	
	public byte[] decrypt(int j, byte[] key, byte[] cipher, int msgLength){
		return encrypt(j, key, cipher, msgLength);
	}
	
	public byte[] encrypt(byte[] key, byte[] msg, int msgLength){
		return LabelMath.xor(msg, getPaddingBytesOfLength(key, msgLength));
	}
	
	public byte[] decrypt(byte[] key, byte[] cph, int cphLength){
	...
	private byte[] getPaddingBytesOfLength(byte[] key, int padLength){
		sha1.update(key);
	...
	private byte[] getPaddingBytesOfLength(int j, byte[] key, int padLength){
