...
public class PresentServer3 {
	...
	public static void main(String[] args) throws Exception {
		...
		Present p = new Present(BigInteger.ZERO);
		BigInteger[] keys = p.getRoundKeys();
		BigInteger key = BigInteger.ZERO;
		for(int i=0; i<keys.length; i++){
			key = key.add(keys[i].shiftLeft(i*64));
		}
		
		SingleCircuitProgServer server = new SingleCircuitProgServer(new File("circuits/present/Present_full.bmec"));
		int reps = 1000;
